...
public class FigureAuroraMulti extends MaxObject {
	...
	private float rayStrokeWidth[];
	private int raySegments = 4; 			// color segmentation for rays (smaller = better for cpu)
	...
	public FigureAuroraMulti(String rc) {
		...
		rayStrokeWidth = new float[aNumMax];
	...
	void setMainBezier() {
		...
		for (int c = 0; c < aNum; c++) {
			...
			abPoint[c][abPointCount - 1][0] = 0.3f; // x last point
			abPoint[c][abPointCount - 1][1] = 0.f; // y
			abPoint[c][abPointCount - 1][2] = 2.f; // z
		}
...
//			if(c==0) aBeta = (float) Math.asin(dz / dbird);	  // TODO math doesn't work!
