...
public class FigureRainbowBezierSimple extends MaxObject {
	...
	public FigureRainbowBezierSimple() {
	...
	public FigureRainbowBezierSimple(String c) {
...
//		else sketch.call("glpolygonmode", new Atom[]{Atom.newAtom("front_and_back"),Atom.newAtom("fill")});
		...
		for(int b=0; b<_bands; b++) {
			...
			for(float t=0.0f; t<1.0f; t+=_add) {
				...
				if(varyScale) sketch.call("gllinewidth", pScale[tToIndex(t)]*_sr);
				
//				if(varyColor) 
				
				float[] _p = P(t, _bp);			// returns array with xyz of main point
				float[] _p2 = P(t+_add, _bp);	// get next point to be able to calculate offset
			...
			float t=1.0f;
			if(varyScale) sketch.call("gllinewidth", pScale[tToIndex(t)]*_sr);
			float[] _p = P(t, _bp);	// returns array with xyz of point
			float[] _p2 = P(t-_add, _bp);
